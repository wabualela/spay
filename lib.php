...
/**
 * SPAY Sudani Payment enrolment plugin.
 *
 * @package    enrol_spay
...
/**
 * SPAY Sudani Payment enrolment plugin implementation.
 * @author Petr Skoda
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_spay_plugin extends enrol_plugin
{
    ...
    public function get_info_icons(array $instances)
    {
        ...
        foreach ($instances as $instance) {
            if ($this->can_spay_enrol($instance, false) !== true) {
                // User can not enrol himspay.
                ...
                // would hide spay-enrolment icons from guests.
                continue;
            }
        }
        $icons = array();
        $icons[] = new pix_icon('icon', get_string('pluginname', 'enrol_spay'), 'enrol_spay');
        return $icons;
    }
    ...
    public function show_enrolme_link(stdClass $instance)
    {
        ...
        if (true !== $this->can_spay_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid)
    {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/spay:config', $context)) {
            return false;
        }
    ...
    /**
     * SPAY Sudani Payment enrol user to course
     *
...
<<<<<<< HEAD
        ...
        $record = $DB->get_record('enrol_spay', array('instanceid' => $instance->id),'*', IGNORE_MISSING);
...
>>>>>>> d8668c00992f3283667ddcdaf0998e32deb4dbb2
            ...
            $new->id = $record->id;
            $new->pin = $data->pin;
            $new->subscriptiondate = time();
<<<<<<< HEAD
            $DB->update_record('enrol_spay', $new, false);
...
=======
            $DB->insrt_record('enrol_spay', $new);
            $timestart = time();
...
>>>>>>> d8668c00992f3283667ddcdaf0998e32deb4dbb2
    ...
    public function enrol_page_hook(stdClass $instance)
    {
        ...
        $enrolstatus = $this->can_spay_enrol($instance);

        if (true === $enrolstatus) {
            // This user can spay enrol using this instance.
            $form = new enrol_spay_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_spay($instance, $data);
                }
            }
        } else {
            // This user can not spay enrol using this instance. Using an empty form to keep
            // the UI consistent with other enrolment plugins that returns a form.
            $data = new stdClass();
            $data->header = $this->get_instance_name($instance);
            $data->info = $enrolstatus;

            // The can_spay_enrol call returns a button to the login page if the user is a
            // guest, setting the login url to the form if that is the case.
            $url = isguestuser() ? get_login_url() : null;
            $form = new enrol_spay_empty_form($url, $data);
        }
     ...
     */
    public function can_spay_enrol(stdClass $instance, $checkuserenrolment = true)
    {
        ...
        if ($checkuserenrolment) {
            ...
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_spay');
            }
        }
        ...
        /*  if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_spay', userdate($instance->enrolstartdate));
        } */

        /* if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_spay', userdate($instance->enrolenddate));
        } */
        ...
        // i move out thise link of code and hardcode max enrol to 1
        $count = $DB->count_records('user_enrolments', array('enrolid' => $instance->id));
        if ($count >= 1) {
            // Bad luck, no more spay enrolments here.
            return get_string('maxenrolledreached', 'enrol_spay');
        }
        ...
        //         return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_spay', $a));
        //     }
        // }
    ...
    public function get_enrol_info(stdClass $instance)
    {
        ...
        $instanceinfo->status = $this->can_spay_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_spay');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_spay_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user)
    {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_spay', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_spay', format_string($course->fullname, true, array('context' => $context)));
    ...
    public function sync(progress_trace $trace, $courseid = null)
    {
        ...
        $trace->output('Verifying spay-enrolments...');
        ...
        // Note: the logic of spay enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'spay' AND e.customint2 > 0)
        ...
        $trace->output('...user spay-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for spay enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/spay:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid)
    {
        ...
        if ($users = get_enrolled_users($context, 'enrol/spay:manage')) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent spay enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid)
    {
        ...
        // we do not use component in manual or spay enrol.
    ...
    {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/spay:config', $context);
    }
    ...
    public function can_hide_show_instance($instance)
    {
        ...
        if (!has_capability('enrol/spay:config', $context)) {
            return false;
        }
    ...
    /**
     * The spay enrollment plugin has several bulk operations that can be performed.
    ...
    public function get_bulk_operations(course_enrolment_manager $manager)
    {
        ...
        if (has_capability("enrol/spay:manage", $context)) {
            $bulkoperations['editselectedusers'] = new enrol_spay_editselectedusers_operation($manager, $this);
        }
        if (has_capability("enrol/spay:unenrol", $context)) {
            $bulkoperations['deleteselectedusers'] = new enrol_spay_deleteselectedusers_operation($manager, $this);
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context)
    {
        ...
        $services = array();
        $servicenames = array();
        foreach (explode("\n", $this->get_config('servicecodes')) as $service) {
        ...
        $mform->addElement('select', 'name', get_string('custominstancename', 'enrol'), $servicenames);
        ...
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_spay'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_spay');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);
        */

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_spay'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_spay');
        ...
        */
        ...
        /*
        $options = array('optional' => false, 'defaultunit' => 3600);
        ...
        */
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_spay'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_spay');
        */

        /* 
        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_spay'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_spay');
        */

        /* 
        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_spay'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_spay');
        */

        /* 
        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_spay'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_spay');
        ...
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_spay');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_spay'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_spay');

        if (enrol_accessing_via_instance($instance)) {
            $warntext = get_string('instanceeditspaywarningtext', 'core_enrol');
            $mform->addElement('static', 'spaywarn', get_string('instanceeditspaywarning', 'core_enrol'), $warntext);
        }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for spay-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for spay-enrolment
    ...
    public function get_welcome_email_contact($sendoption, $context)
    {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/spay:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/spay:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
