...
/**
 * SPAY Sudani Payment enrolment plugin tests.
 *
 * @package    enrol_spay
 * @category   phpunit
...
global $CFG;
require_once($CFG->dirroot.'/enrol/spay/lib.php');
require_once($CFG->dirroot.'/enrol/spay/locallib.php');

class enrol_spay_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('spay'));
        $plugin = enrol_get_plugin('spay');
        $this->assertInstanceOf('enrol_spay_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_spay', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_spay', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $spayplugin = enrol_get_plugin('spay');

        $trace = new null_progress_trace();

        // Just make sure the sync does not throw any errors when nothing to do.
        $spayplugin->sync($trace, null);
        $spayplugin->sync($trace, $SITE->id);
    }
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'spay')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $id = $spayplugin->add_instance($course3, array('status'=>ENROL_INSTANCE_ENABLED, 'roleid'=>$teacherrole->id));
        ...
        $spayplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $spayplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $spayplugin->enrol_user($instance1, $user3->id, $studentrole->id);
        ...
        $spayplugin->enrol_user($instance3, $user1->id, $studentrole->id);
        $spayplugin->enrol_user($instance3, $user2->id, $studentrole->id);
        $spayplugin->enrol_user($instance3, $user3->id, $studentrole->id);
        $spayplugin->enrol_user($instance3b, $user1->id, $teacherrole->id);
        $spayplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $spayplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
    ...
    public function test_expired() {
        ...
        $spayplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $spayplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $spayplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $spayplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $spayplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now-60*60);
        $spayplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now+60*60);
        $spayplugin->enrol_user($instance3b, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $spayplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $spayplugin->get_config('expiredaction'));
        $spayplugin->sync($trace, null);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));


        $spayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $spayplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        ...
        $spayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $spayplugin enrol_spay_plugin */
        $spayplugin = enrol_get_plugin('spay');
        ...
        $spayplugin->set_config('expirynotifylast', $now - 60*60*24);
        $spayplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'spay')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        ...
        $spayplugin->enrol_user($instance1, $user1->id, $studentrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $spayplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $spayplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $spayplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $spayplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $spayplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        ...
        $spayplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $spayplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $spayplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance3, $user1->id, $editingteacherrole->id);
        $spayplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $spayplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance4, $user4->id, $editingteacherrole->id);
        $spayplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $spayplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $spayplugin->send_expiry_notifications($trace);
        ...
        $spayplugin->set_config('expirynotifylast', time() - 60*60*24);
        $spayplugin->set_config('expirynotifyhour', '24');
    ...
    public function test_show_enrolme_link() {
        ...
        $DB->update_record('enrol', $instance1);
        $spayplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);

        // New enrolments are not allowed, but enrolment instance is enabled.
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance2->customint6 = 0;
        $DB->update_record('enrol', $instance2);
        $spayplugin->update_status($instance2, ENROL_INSTANCE_ENABLED);

        // New enrolments are allowed , but enrolment instance is disabled.
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance3->customint6 = 1;
        $DB->update_record('enrol', $instance3);
        $spayplugin->update_status($instance3, ENROL_INSTANCE_DISABLED);

        // New enrolments are not allowed and enrolment instance is disabled.
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance4->customint6 = 0;
        $DB->update_record('enrol', $instance4);
        $spayplugin->update_status($instance4, ENROL_INSTANCE_DISABLED);

        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance5->customint6 = 1;
        $instance5->customint5 = $cohort1->id;
        $DB->update_record('enrol', $instance1);
        $spayplugin->update_status($instance5, ENROL_INSTANCE_ENABLED);

        $id = $spayplugin->add_instance($course5, $spayplugin->get_instance_defaults());
        $instance6 = $DB->get_record('enrol', array('id'=>$id), '*', MUST_EXIST);
        $instance6->customint6 = 1;
        $instance6->customint5 = $cohort2->id;
        $DB->update_record('enrol', $instance1);
        $spayplugin->update_status($instance6, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in future.
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance7->customint6 = 1;
        $instance7->enrolstartdate = time() + 60;
        $DB->update_record('enrol', $instance7);
        $spayplugin->update_status($instance7, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in past.
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance8->customint6 = 1;
        $instance8->enrolstartdate = time() - 60;
        $DB->update_record('enrol', $instance8);
        $spayplugin->update_status($instance8, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in future.
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance9->customint6 = 1;
        $instance9->enrolenddate = time() + 60;
        $DB->update_record('enrol', $instance9);
        $spayplugin->update_status($instance9, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in past.
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance10->customint6 = 1;
        $instance10->enrolenddate = time() - 60;
        $DB->update_record('enrol', $instance10);
        $spayplugin->update_status($instance10, ENROL_INSTANCE_ENABLED);

        // Maximum enrolments reached.
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance11->customint6 = 1;
        $instance11->customint3 = 1;
        $DB->update_record('enrol', $instance11);
        $spayplugin->update_status($instance11, ENROL_INSTANCE_ENABLED);
        $spayplugin->enrol_user($instance11, $user2->id, $studentrole->id);

        // Maximum enrolments not reached.
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'spay'), '*', MUST_EXIST);
        $instance12->customint6 = 1;
        $instance12->customint3 = 1;
        $DB->update_record('enrol', $instance12);
        $spayplugin->update_status($instance12, ENROL_INSTANCE_ENABLED);

        $this->setUser($user1);
        $this->assertTrue($spayplugin->show_enrolme_link($instance1));
        $this->assertFalse($spayplugin->show_enrolme_link($instance2));
        $this->assertFalse($spayplugin->show_enrolme_link($instance3));
        $this->assertFalse($spayplugin->show_enrolme_link($instance4));
        $this->assertFalse($spayplugin->show_enrolme_link($instance7));
        $this->assertTrue($spayplugin->show_enrolme_link($instance8));
        $this->assertTrue($spayplugin->show_enrolme_link($instance9));
        $this->assertFalse($spayplugin->show_enrolme_link($instance10));
        $this->assertFalse($spayplugin->show_enrolme_link($instance11));
        $this->assertTrue($spayplugin->show_enrolme_link($instance12));

        require_once("$CFG->dirroot/cohort/lib.php");
        cohort_add_member($cohort1->id, $user1->id);

        $this->assertTrue($spayplugin->show_enrolme_link($instance5));
        $this->assertFalse($spayplugin->show_enrolme_link($instance6));
    }

    /**
     * This will check user enrolment only, rest has been tested in test_show_enrolme_link.
     */
    public function test_can_spay_enrol() {
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_spay');
        ...
        $DB->update_record('enrol', $instance1);
        $spayplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);
        $spayplugin->enrol_user($instance1, $user2->id, $editingteacherrole->id);

        $this->setUser($guest);
        $this->assertContains(get_string('noguestaccess', 'enrol'),
                $spayplugin->can_spay_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($spayplugin->can_spay_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $spayplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $spayplugin->can_spay_enrol($instance1, true));
    }

    /**
     * Test enrol_spay_check_group_enrolment_key
     */
    public function test_enrol_spay_check_group_enrolment_key() {
        global $DB;
        spay::resetAfterTest(true);

        // Test in course with groups.
        $course = spay::getDataGenerator()->create_course(array('groupmode' => SEPARATEGROUPS, 'groupmodeforce' => 1));

        $group1 = $this->getDataGenerator()->create_group(array('courseid' => $course->id));
        $group2 = $this->getDataGenerator()->create_group(array('courseid' => $course->id, 'enrolmentkey' => 'thepassword'));

        $result = enrol_spay_check_group_enrolment_key($course->id, 'invalidpassword');
        $this->assertFalse($result);

        $result = enrol_spay_check_group_enrolment_key($course->id, 'thepassword');
        ...
        // Test without groups.
        $othercourse = spay::getDataGenerator()->create_course();
        $result = enrol_spay_check_group_enrolment_key($othercourse->id, 'thepassword');
        $this->assertFalse($result);
    ...
    public function test_get_welcome_email_contact() {
        ...
        // Enable spay enrolment plugin and set to send email from course contact.
        $spayplugin = enrol_get_plugin('spay');
        $instance1 = $DB->get_record('enrol', ['courseid' => $course1->id, 'enrol' => 'spay'], '*', MUST_EXIST);
        $instance1->customint6 = 1;
        ...
        $contact = $spayplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_COURSE_CONTACT, $context);
        ...
        $spayplugin->enrol_user($instance1, $user1->id, $editingteacherrole->id);
        ...
        $DB->update_record('enrol', $instance1);
        $spayplugin->enrol_user($instance1, $user3->id, $managerrole->id);

        // Give manager role holdkey capability.
        assign_capability('enrol/spay:holdkey', CAP_ALLOW, $managerrole->id, $context);

        // We should get the manager email contact.
        $contact = $spayplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_KEY_HOLDER, $context);
        ...
        $spayplugin->enrol_user($instance1, $user4->id, $managerrole->id);
        $contact = $spayplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_KEY_HOLDER, $context);
        $this->assertEquals($user3->username, $contact->username);
        $this->assertEquals($user3->email, $contact->email);
        ...
        $contact = $spayplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_NOREPLY, $context);
        $this->assertEquals($noreplyuser, $contact);
    }
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the spay enrol plugin.
        ...
        // SPAY Sudani Payment enrol has 2 enrol actions -- edit and unenrol.
