...
/**
 * SPAY Sudani Payment enrol plugin external functions
 *
 * @package    enrol_spay
...
/**
 * SPAY Sudani Payment enrolment external functions.
 *
 * @package   enrol_spay
 ...
 * @since     Moodle 2.6
 */
class enrol_spay_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of spay enrolment plugin.'))
            );
    }

    /**
     * Return spay-enrolment instance information.
     *
     * @param int $instanceid instance id of spay enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        $params = spay::validate_parameters(spay::get_instance_info_parameters(), array('instanceid' => $instanceid));

        // Retrieve spay enrolment plugin.
        $enrolplugin = enrol_get_plugin('spay');
        if (empty($enrolplugin)) {
            throw new moodle_exception('invaliddata', 'error');
        }

        spay::validate_context(context_system::instance());
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of spay enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }

    /**
     * SPAY Sudani Payment enrol the current user in the given course.
     *
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        $params = spay::validate_parameters(spay::enrol_user_parameters(),
                                            array(
                                                'courseid' => $courseid,
                                                'password' => $password,
        ...
        // Retrieve the spay enrolment plugin.
        $enrol = enrol_get_plugin('spay');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_spay');
        }

        // We can expect multiple spay-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "spay") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_spay_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_spay_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_spay')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_spay', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_spay($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
